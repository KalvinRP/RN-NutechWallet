  ...
  return (
    <ScrollView style={styles.bgColor}>
      <KeyboardAvoidingView
        behavior="position">
        <Center>
          <Image source={require('../../assets/splash.png')} alt="loading" style={style.entranceImage} />
        </Center>
        <Text style={style.entranceTitle}>Register</Text>
        <FormControl>
          <Square>
            <Stack space={5} w="85%" mt={30}>
              <Stack>
                <Input type={'text'} value={data.email} onChangeText={(value) => posting("email", value)} form size="lg" variant="outline" bg={style.bgTextInput} p={2} placeholder="Surel" />
                {invalid.email && <Text style={{ color: 'red' }}>Surel tidak berlaku. Harap periksa kembali.</Text>}
              </Stack>
              <Stack>
                <Input type={'text'} value={data.first_name} onChangeText={(value) => posting("first_name", value)} size="lg" variant="outline" bg={style.bgTextInput} p={2} placeholder="Nama Depan" />
              </Stack>
              <Stack>
                <Input type={'text'} value={data.last_name} onChangeText={(value) => posting("last_name", value)} size="lg" variant="outline" bg={style.bgTextInput} p={2} placeholder="Nama Belakang" />
              </Stack>
              <Stack>
                <Input type={'password'} value={data.password} onChangeText={(value) => posting("password", value)} size="lg" variant="outline" bg={style.bgTextInput} p={2} placeholder="Sandi" />
